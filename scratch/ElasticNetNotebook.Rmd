...
Below, we are performing a basic linear regression predicting the mean valence rating on each IAPS image given subjects' positive and negative ratings. This time, no cross validation is used. We are fitting this model on the training dataset (36 of 48 subjects) and evaluating the fit.
...
In order to set up the elastic net model, we split the data into 75% training and 25% testing. That is, 36 of the 48 subjects are in the training data. Below is the code to describe how we perform 6 fold cross validation predicting the mean valence of the IAPS images given subjects' positive and negative ratings. We tune the model, testing every combination of penalty and mixture (lambda and alpha) as defined in the object `netGrid`.
...
Below, we are performing a basic linear regression predicting the mean arousal rating on each IAPS image given subjects' positive and negative ratings. This time, no cross validation is used. We are fitting this model on the training dataset (36 of 48 subjects) and evaluating the fit.
